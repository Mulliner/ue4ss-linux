...
#else
    #define MINIMAL_WINDOWS_API extern "C" __attribute__((visibility("default")))
#endif
...
namespace RC::Unreal
{
  ...
  namespace Windows
  {
    ...
    MINIMAL_WINDOWS_API HMODULE WINAPI GetModuleHandleW(LPCTSTR lpModuleName);
    ...
    {
      return Windows::TryAcquireSRWLockExclusive((PSRWLOCK)SRWLock);
    }

    FORCEINLINE BOOLEAN WINAPI TryAcquireSRWLockShared(SRWLOCK* SRWLock)
    {
      return Windows::TryAcquireSRWLockShared((PSRWLOCK)SRWLock);
    }
