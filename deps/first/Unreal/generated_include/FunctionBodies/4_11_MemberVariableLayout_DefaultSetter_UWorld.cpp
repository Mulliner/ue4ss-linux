...
{
    UWorld::MemberOffsets.emplace(STR("URL"), 0x6F0);
}
...
{
    UWorld::MemberOffsets.emplace(STR("AsyncPreRegisterLevelStreamingTasks"), 0x930);
}

if (auto it = UWorld::MemberOffsets.find(STR("UEP_TotalSize")); it == UWorld::MemberOffsets.end())
{
    UWorld::MemberOffsets.emplace(STR("UEP_TotalSize"), 0x940);
}
