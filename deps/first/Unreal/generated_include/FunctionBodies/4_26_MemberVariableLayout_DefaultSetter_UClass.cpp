...
if (auto it = UClass::MemberOffsets.find(STR("bCooked")); it == UClass::MemberOffsets.end())
{
    ...
    UClass::BitfieldInfos.emplace(STR("bCooked"), BitfieldInfo{31, 1, 1});
}
...
{
    UClass::MemberOffsets.emplace(STR("ClassCastFlags"), 0xD0);
}
...
{
    UClass::MemberOffsets.emplace(STR("ReferenceTokenStream"), 0x1E8);
}
