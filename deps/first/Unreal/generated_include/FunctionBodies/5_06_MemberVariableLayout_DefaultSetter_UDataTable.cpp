...
if (auto it = UDataTable::MemberOffsets.find(STR("bIgnoreExtraFields")); it == UDataTable::MemberOffsets.end())
{
    ...
    UDataTable::BitfieldInfos.emplace(STR("bIgnoreExtraFields"), BitfieldInfo{1, 1, 1});
}
...
if (auto it = UDataTable::MemberOffsets.find(STR("bPreserveExistingValues")); it == UDataTable::MemberOffsets.end())
{
    UDataTable::MemberOffsets.emplace(STR("bPreserveExistingValues"), 0x80);
    UDataTable::BitfieldInfos.emplace(STR("bPreserveExistingValues"), BitfieldInfo{3, 1, 1});
}
