...
{
    try {
        auto utf8_val = to_utf8_string(val_str);
        int32_t offset = std::stoi(utf8_val, nullptr, 0);
        offsets.emplace(member_name, offset);
        size_t colon1 = val_str.find(u':');
        if (colon1 != File::StringType::npos && colon1 + 1 < val_str.size()) {
            uint8_t bit_pos = static_cast<uint8_t>(std::stoi(to_utf8_string(val_str.substr(colon1 + 1))));
            uint8_t bit_len = 1;
            uint8_t storage_size = 1;
            size_t colon2 = val_str.find(u':', colon1 + 1);
            if (colon2 != File::StringType::npos && colon2 + 1 < val_str.size()) {
                bit_len = static_cast<uint8_t>(std::stoi(to_utf8_string(val_str.substr(colon2 + 1))));
                size_t colon3 = val_str.find(u':', colon2 + 1);
                if (colon3 != File::StringType::npos && colon3 + 1 < val_str.size()) {
                    storage_size = static_cast<uint8_t>(std::stoi(to_utf8_string(val_str.substr(colon3 + 1))));
                }
            }
