                ...
                if (!std::filesystem::exists(status_scripts_dir))
                {
                    ...
                    if (!ec)
                    {
                        ...
                        if (main_lua.is_open())
                        {
                            ...
                            main_lua << "local function LogUE4SSActive()\n";
                            main_lua << "    print('[UE4SS] UE4SS is active and running!')\n";
                            main_lua << "end\n\n";
                            main_lua << "-- Hook into InitGameState to log when game state initializes\n";
                            main_lua << "RegisterInitGameStatePostHook(function()\n";
                            main_lua << "    LogUE4SSActive()\n";
                            main_lua << "end)\n\n";
                            main_lua << "-- Hook into BeginPlay for actors\n";
                            main_lua << "RegisterBeginPlayPostHook(function(actor)\n";
                            main_lua << "    -- Only log once for the GameMode actor\n";
                            main_lua << "    if not _G.UE4SS_Logged then\n";
                            main_lua << "        _G.UE4SS_Logged = true\n";
                            main_lua << "        LogUE4SSActive()\n";
                            main_lua << "    end\n";
                            main_lua << "end)\n";
                            main_lua.close();
